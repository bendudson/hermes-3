...
of mean power flux each with a +/-10% sinusoidal fluctuation of a period of 0.5ms. The density source has a mean of zero and 
oscillates between :math:`-1\times10^{22}` and :math:`1\times10^{22}`, also with a period of 0.5ms.
...
.. code-block:: ini
   ...
   source_prefactor = sin((2/0.5)*pi*1e6*t)   #  Oscillation between -1 and 1, period 0.5ms
   ...
   source_time_dependent = true
   source_prefactor = 1 + 0.1 * sin((2/0.5)*pi*1e6*t)   #  10% fluctuation on on  top of background source, period 0.5ms
